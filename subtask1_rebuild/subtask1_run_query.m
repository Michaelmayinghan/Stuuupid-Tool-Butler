...
% Ensure rebuild helpers are preferred during nested function calls.
...
switch qId
    ...
    case 4
        ...
        for i = 1:numel(nearestIdx)
            result.answerTable = [result.answerTable; {sprintf('#%d', i), sprintf('%s (%.2f m)', string(ctx.keyP.Remark(nearestIdx(i))), nearestDist(i))}]; %#ok<AGROW>
        end
...
while ~isempty(remaining)
    ...
    if numel(fullPath) > 0 && numel(bestPath) > 1
        fullPath = [fullPath, bestPath(2:end)]; %#ok<AGROW>
    else
        fullPath = [fullPath, bestPath]; %#ok<AGROW>
    end
    totalCost = totalCost + bestCost;
    currentNode = i_findAnyNodeByName(ctx.nodes, keyNames(bestIdx));
    visitOrder(end+1) = currentNode; %#ok<AGROW>
...
if numel(backPath) > 1
    fullPath = [fullPath, backPath(2:end)]; %#ok<AGROW>
end
